...
export class EventComp extends Component {
    ...
    constructor(props) {
        ...
        // 메소드에 .bind로 묶어서 this 전달 아래 this.printEvent는 작성한 메소드 그 메소드에 .bind(this)로 연결
        this.printEvent = this
            .printEvent
            .bind(this);
        //this.printEvent 이름 참고) let num = 0; num = num+1; >> num의 결과 :1

        this.printAdrress = this
            .printAdrress
            .bind(this);
        this.setToggle = this
            .setToggle
    ...
    // 이벤트안에서 작성한 함수 그대로 들고와서 사용할수 있다 render에서 사용한 값을 쓸수 없다 this.state.name을 통해서 접근
    // 메소드에 바로 bind를 통해서 this를 묶어줄 수 없다 메소드만 만들어서 바로 사용하면 this를 찾지못해서 오류 >>
    ...
    changeColor(e) {
        // e : 이벤트 객체를 들고와서 e.type을 통해서 이벤트 구분가능 이벤트의type에 따라서 setState에 값 다르게 넣을수있습니다
        // onMouseLeave에 changeColor를 추가하여 onMouseLeave이벤트가 발생했을때는 color:""
    ...
    // 화살표 함수를 가지는 메소드 메소드 이름에 화살표 함수 작성
    arrowPrint = () => {
        console.log("이벤트 실행");
        console.log(this.state.name);
    }
    ...
    // onChange 공용메소드
    onInputChange = (e) => {
        // inputNickname을 그대로 사용 > inputNickname에만 값이 들어감 e.target.name : name속성값을 가져와서
        // 사용 변수값을 사용할려면 [] 사용가능
        this.setState({
    ...
    render() {
        // render안에서 this = EventComp; this.state는 construct의 속성값
        ...
        <div>
            ...
            <p>
                버튼을 눌렀을때 console.log(이벤트 출력) this.state.name "홍길동"을 출력
            </p>
            <button
                onClick={function () {
                    console.log("이벤트 출력");
                    // name값을 render에서 this.state값을 가져와서 this를 연결하지않아도 사용가능
                    console.log(name);
                }.bind(this)}>
                메소드를 사용한 이벤트
            </button>
            ...
            {/** 이벤트 : 메소드 만들기 실습 */}
            <p>
                버튼을 누르면 state의 adrress 부산을 출력하고 console.log를 이용하여 이벤트 완료 출력
            </p>
            <button onClick={this.printAdrress}>
                이벤트
            </button>
         ...
         */
            ...
            <input
                ...
                onChange={(e) => {
                    ...
                    // setState는 비동기로 움직임으로 바로 state에 접근해서 값을 출력하면 이전값이 나온다
                    console.log("input", this.state.input);
                }}/> {/** change 공용메소드(함수) 만들기 : 사용하지않아도 상관x */}
            <h3>input 2개에서 값 받아오기</h3>
            <p>inputNickname의 값 : {this.state.inputNickname}
            </p>
            <input name='inputNickname'
                // state의 속성이름과 동일
                type="text" onChange={this.onInputChange}/>
            <p>inputBook의 값 : {this.state.inputBook}</p>
            <input name='inputBook' type="text" onChange={this.onInputChange}/>
